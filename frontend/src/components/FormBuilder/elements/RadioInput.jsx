          ...
          <Grid container spacing={1}>
            <Grid item xs={9}>
              ...
              <RadioGroup
                value={selectedOption}
                onChange={(e) => handleOptionChange(item.id, e.target.value)}
                disabled={disabled} // Disable the RadioGroup if disabled prop is true
              >
                {item.options && item.options.length > 0 && item.options.map((opt, key) => (
                  <Box sx={{ display: "flex", alignItems: "center" }} key={opt.id}>
                    <FormControlLabel
                      value={opt.id}
                      control={<Radio disabled={disabled} />} // Disable the Radio if disabled prop is true
                      label={
                        <TextField
                          variant="outlined"
                          fullWidth
              ...
              <Button variant="text" onClick={() => createNewOption(item.id)} disabled={disabled}>
                Add Option
              </Button>
            </Grid>
            <Grid item xs={3}>
